...
const JobDetails = () => {
  ...
  // Local States
  const [selectedGig, setSelectedGig] = useState(reduxGig?._id === id ? reduxGig : null);
    ...
    <div className="bg-slate-50 min-h-screen pb-20">
      ...
      <div className="max-w-7xl mx-auto px-4 py-10">
        ...
        <div className="flex justify-between items-center mb-8">
          ...
          <span className="text-xs font-black text-slate-300 uppercase tracking-[0.2em]">Job ID: {id.slice(-6)}</span>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 items-start">
          ...
          <div className="lg:col-span-2 space-y-8">
            <div className="bg-white p-3 rounded-[40px] shadow-sm border border-slate-100">
              ...
              <div className="flex gap-4 p-4 overflow-x-auto no-scrollbar">
                {[selectedGig?.thumbnail, ...(selectedGig?.images || [])].map((img, i) => (
                  <img
                    key={i}
                    src={img}
            ...
            <div className="bg-white rounded-[40px] p-8 shadow-sm border border-slate-100">
              ...
              <h1 className="text-4xl font-black text-slate-900 mb-6 leading-tight">{selectedGig?.title}</h1>
              <div className="flex flex-wrap gap-6 mb-10 pb-8 border-b border-slate-50">
                <div className="flex items-center gap-2 text-slate-500 font-medium"><MapPin size={18} className="text-indigo-500" /> {selectedGig?.location}</div>
                <div className="flex items-center gap-2 text-slate-900 font-bold"><Star size={18} className="text-amber-500" fill="currentColor" /> {selectedGig?.rating} <span className="text-slate-400 font-medium">({selectedGig?.totalReviews} reviews)</span></div>
              </div>
              <h3 className="text-xl font-bold text-slate-900 mb-4">Service Description</h3>
              <p className="text-slate-600 leading-relaxed text-lg whitespace-pre-line">{selectedGig?.description}</p>
            </div>
          </div>
          ...
          <div className="space-y-6 lg:sticky lg:top-24">
            ...
            <div className="bg-white rounded-[40px] p-8 shadow-xl shadow-slate-200/50 border border-indigo-50">
              <div className="flex justify-between items-end mb-8">
                <div>
                  ...
                  <h2 className="text-5xl font-black text-slate-950 tracking-tighter">${selectedGig?.price}</h2>
                </div>
                <span className="bg-slate-50 px-4 py-2 rounded-xl text-xs font-black text-slate-500 uppercase border border-slate-100">{selectedGig?.priceType}</span>
              </div>

              <div className="space-y-4 mb-10">
                <div className="flex items-center gap-3 text-slate-600 bg-slate-50 p-4 rounded-2xl">
                  <Clock size={20} className="text-indigo-500" />
                  <span className="text-xs font-bold uppercase tracking-tight">On-Time Delivery</span>
                </div>
                <div className="flex items-center gap-3 text-slate-600 bg-slate-50 p-4 rounded-2xl">
              ...
              <div className="space-y-3">
                <button onClick={handleBooking} className="w-full bg-indigo-600 text-white py-5 rounded-[20px] font-black text-lg shadow-lg shadow-indigo-100 hover:bg-indigo-700 hover:-translate-y-1 transition-all flex items-center justify-center gap-3">
                ...
                <div className="flex items-center gap-3">
                  <div className="w-12 h-12 bg-white rounded-2xl flex items-center justify-center text-indigo-600 font-black shadow-sm overflow-hidden">
                    {selectedGig?.seller?.profilePic ? (
                        <img src={selectedGig.seller.profilePic} className="w-full h-full object-cover" alt="S" />
                    ) : (
                  ...
                  <div>
                    ...
                    <p className="text-[10px] text-slate-400 font-bold uppercase">Pro Seller</p>
                  </div>
                </div>
                <button 
                  onClick={() => navigate(`/seller-profile/${selectedGig?.seller?._id}`)}
            ...
            <div className="text-center">
               <p className="text-[10px] text-slate-300 font-bold uppercase tracking-widest">Powered by QuickService</p>
            </div>
